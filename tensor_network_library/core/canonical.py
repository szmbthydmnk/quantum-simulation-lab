...
def _is_left_orthonormal(A: np.ndarray, atol: float = 1e-10) -> bool:
...
def _is_right_orthonormal(A: np.ndarray, atol: float = 1e-10) -> bool:
...
def _update_tensor(mps: MPS, site: int, data: np.ndarray) -> None:
...
def left_canonicalize(
    ...
    """Bring an MPS into left-canonical form via QR sweeps.
    ...
    for i in range(mps.L - 1):
        ...
        B_new = np.tensordot(R, B, axes=([1], [0]))  # (chi_new, d', chi_R')
...
# ---------------------------------------------------------------------------
# Convenience / validation helpers (public)
# ---------------------------------------------------------------------------
